...
const EditProduct: React.FC = () => {
  ...
  const fetchPromotions = useCallback(async () => {
    ...
    try {
      ...
      showErrorAlert(
        t("dashboardScreens.productDetails.errorLoadingPromotions"),
        (error as { message: string }).message ||
          t("dashboardScreens.productDetails.errorLoadingPromotions")
      );
    } finally {
        ...
        <div className="p-6 overflow-y-auto max-h-[calc(100vh-150px)]">
          ...
          <div className="mb-6 animate-fade-in">
            ...
            {categoriesError && (
              ...
              >
                {t("dashboardScreens.productDetails.retryLoadingCategories")}
              </button>
            )}
          </div>
          ...
          {promotions.length > 0 && (
            <div className="mb-6 animate-fade-in">
              <label className="block text-sm font-medium text-gray-700 mb-2">
                {t("dashboardScreens.productDetails.currentPromotion")}
              </label>
              {isLoadingPromotions ? (
                <div className="flex justify-center py-4">
                  <div className="animate-spin rounded-full h-6 w-6 border-b-2 border-indigo-600"></div>
                ...
                <div className="flex items-center gap-2">
                  ...
                  >
                    {t("dashboardScreens.productDetails.seeDetails")}
                  </button>
                </div>
              )}
            </div>
              ...
              <div className="bg-white rounded-lg max-w-md w-full max-h-[80vh] overflow-y-auto">
                <div className="p-6">
                  <div className="flex justify-between items-center mb-4">
                    ...
                    >
                      {selectedPromotion.isActive
                        ? t("dashboardScreens.productDetails.active")
                        : t("dashboardScreens.productDetails.inactive")}
                    </div>
                  </div>

                  <div className="mb-4">
                    <h4 className="font-medium text-gray-700 mb-2">
                      {t("dashboardScreens.productDetails.promotionPeriod")}
                    </h4>
                    <div className="text-sm text-gray-600">
                      <p>
                        {t("dashboardScreens.productDetails.from")}{" "}
                        {new Date(
                          selectedPromotion.startDate
                        ).toLocaleDateString()}
                      </p>
                      <p>
                        {t("dashboardScreens.productDetails.to")}{" "}
                        {new Date(
                  ...
                  <div className="mb-4">
                    <h4 className="font-medium text-gray-700 mb-2">
                      {t("dashboardScreens.productDetails.discountDetails")}
                    </h4>
                    {isPromotionApplicableToProduct(
                      ...
                      <div className="space-y-2">
                        {selectedPromotion.actions.map((action, index) => (
                          <div
                            key={index}
                            className="bg-blue-100 text-blue-800 px-3 py-1 rounded-full text-sm inline-block mr-2"
                          >
                            {action.actionType === "PERCENT_OFF" &&
                              `${action.actionValue?.percentage}% OFF`}
                            {action.actionType === "FIXED_OFF" &&
                              `€${action.actionValue?.amount} OFF`}
                          </div>
                        ))}
                      </div>
                    ) : (
                      <p className="text-gray-500 italic">
                        {t(
                          "dashboardScreens.productDetails.noDiscountApplicable"
                        )}
                      </p>
                    )}
                  </div>
                  ...
                  >
                    {t("dashboardScreens.productDetails.close")}
                  </button>
                </div>
              </div>
            </div>
